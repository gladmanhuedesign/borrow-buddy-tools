-- Update the tool status trigger to handle overdue requests properly
...
BEGIN
  ...
  -- When a request becomes overdue
  IF NEW.status = 'overdue' AND OLD.status != 'overdue' THEN
    -- If it was picked up, keep it as borrowed
    IF NEW.picked_up_at IS NOT NULL THEN
      UPDATE public.tools 
      SET status = 'borrowed' 
      WHERE id = NEW.tool_id;
    ELSE
      -- If it was never picked up, make it available again
      UPDATE public.tools 
